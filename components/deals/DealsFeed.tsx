...
export default function DealsFeed({
  ...
  initialPage: DealsPage;
  sort: string;
  q?: string;
  category?: string;
  serverLayout: ViewLayout;
 ...
 const storeLayout = useUIStore((state) => state.layout);
  const storeExpanded = useUIStore((state) => state.isExpanded);

  const [hydrated, setHydrated] = useState(false);
    // eslint-disable-next-line react-hooks/set-state-in-effect
    useEffect(() => { setHydrated(true); }, []);

const layout = hydrated ? storeLayout : serverLayout;
const isExpanded = hydrated ? storeExpanded : serverExpanded;
