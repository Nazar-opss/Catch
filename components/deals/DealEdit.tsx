...
function dealToFormValues(deal: EditableDeal): DealFormValues {
  return {
    ...
    expiresAt: deal.expiresAt
      ? new Date(deal.expiresAt).toISOString().slice(0, 10)
      : "",
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel>
                    Зображення (
                    {field.value?.length + existingImages.length || 0}/5)
                  </FieldLabel>
                    ...
                    >
                      {existingImages?.map((url, index) => (
                        <Tooltip key={url}>
                          <TooltipTrigger asChild>
                            ...
                            >
                              ...
                              <Button
                                type="button"
                                size="icon"
                                className="size-6 absolute top-1 right-1 rounded-full p-1 cursor-pointer bg-secondary text-muted-foreground hover:text-card-foreground hover:bg-secondary hover:ring ring-primary transition-colors duration-200"
                                onClick={() =>
                                  form.setValue(
                                    "existingImages",
                              ...
                              >
                                <X className="text-red-600 hover:text-red-700" />
                          ...
                          >
                            <p className="text-xs">Image #{index}</p>
                          </TooltipContent>
                        </Tooltip>
                      ))}
                          ...
                          <TooltipTrigger asChild>
                            ...
                            >
                              ...
                              <FileUploadItemDelete asChild>
                                <Button
                                  size="icon"
                                  className="size-6 absolute top-1 right-1 rounded-full p-1 cursor-pointer text-muted-foreground bg-secondary hover:text-card-foreground hover:bg-card hover:ring ring-primary transition-colors duration-200"
                                >
                                  <X className="text-red-600 hover:text-red-700" />
                                  <span className="sr-only">Delete</span>
                                </Button>
                      ...
                      <FileUploadDropzone
                        ...
                        className="w-20 h-20 cursor-pointer group transition-colors duration-200 hover:border-orange-400 p-0 rounded-xl border-border"
                      >
