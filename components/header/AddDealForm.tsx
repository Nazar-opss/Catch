...
export default function AddDealForm({
    ...
    startTransition(async () => {
      ...
      if (values.images.length > 0) {
        ...
        try {
          ...
          if (!uploadResult.ok || !Array.isArray(data.urls)) {
            form.setError("images", {
              type: "server",
              message: data.error || "Не вдалося завантажити зображення",
            });
            return;
          }
        ...
        >
          ...
          <div className="flex items-center w-full bg-secondary rounded-xl p-1">
            <button
              ...
              className={`flex cursor-pointer transition-all duration-200 gap-2 px-4 p-1.5 w-full font-medium justify-center items-center text-[13px] rounded-lg ${dealType === "online" ? "bg-[#ea580c] text-white shadow-sm" : "text-muted-foreground"} `}
            >
            ...
            <button
              ...
              aria-disabled
              title="Скоро"
              className="flex cursor-not-allowed opacity-50 gap-2 px-4 transition-all duration-200 p-1.5 font-medium w-full justify-center items-center text-[13px] rounded-lg text-muted-foreground"
            >
          ...
          <DialogFooter>
            <DialogClose asChild onClick={() => form.reset()}>
              <Button
                variant="outline"
                className="rounded-lg h-10 px-5 py-2 text-[14px] font-semibold cursor-pointer border border-border"
              >
                Скасувати
              </Button>
            </DialogClose>
            ...
            >
                {isPending === true ? "Опубліковуємо..." : "Опублікувати"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle className="text-xl font-bold text-card-foreground tracking-tight">
            Додати нову знижку
          </DrawerTitle>
        </DrawerHeader>
        <div className="no-scrollbar max-h-[calc(100vh-2rem)] overflow-y-auto p-2 sm:p-8">
          <DealFormContent form={form} dealType={dealType} />
        </div>
        <DrawerFooter>
          <DrawerClose asChild>
