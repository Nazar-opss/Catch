...
const passwordErrorMap: Record<string, string> = {
  ...
  CREDENTIAL_ACCOUNT_NOT_FOUND:
    "Для цього акаунта не встановлено пароль, спробуйте увійти через соцмережу або відновити пароль",
};
...
export default function ProfileSettings({
  ...
  const createPasswordForm = useForm<NewPasswordSchemaValues>({
  ...
  const handleChangeEmail = async () => {
    ...
    if (!valid) return;
    const newEmail = String(form.getValues("email") ?? "")
      .trim()
      .toLowerCase();
    ...
    if (error) {
      toast.error(
        emailErrorMap[error.code ?? ""] ?? "Не вдалося змінити пошту",
      );
      return;
    }
  ...
  const handleChangePassword = async () => {
    ...
    if (error) {
      toast.error(
        passwordErrorMap[error.code ?? ""] ?? "Не вдалося змінити пароль",
      );
      return;
    }
  ...
  const handleCreatePassword = async () => {
    const valid = await createPasswordForm.trigger([
      "password",
      "confirmPassword",
    ]);
    if (!valid) return;
    const password = createPasswordForm.getValues("password");
    const confirmPassword = createPasswordForm.getValues("confirmPassword");

    setSavingField("createPassword");
    const { error } = await createPassword({
      password,
      confirmPassword,
    });
    setSavingField(null);
    ...
    if (error) {
      toast.error(
        passwordErrorMap[error ?? ""] ?? "Не вдалося створити пароль",
      );
      return;
    }
    ...
    toast.success("Пароль оновлено");
    router.refresh()
    createPasswordForm.resetField("password", { defaultValue: "" });
    createPasswordForm.resetField("confirmPassword", { defaultValue: "" });
  };
    ...
    <div className=" bg-card rounded-[24px] border border-border shadow-sm p-8 flex flex-col">
      ...
      <div className="space-y-4 border-b border-secondary pb-8">
          ...
          render={({ field, fieldState }) => (
            ...
            >
              ...
              <FieldGroup className="flex flex-row gap-3 items-center">
                ...
                />
                <Button
                  disabled={!fieldState.isDirty || savingField === "name"}
                  onClick={saveField("name")}
                  className="bg-card h-full border-border text-muted-foreground cursor-pointer hover:bg-transparent px-5 py-2.5 rounded-xl font-medium text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  variant={"outline"}
                >
                  Зберегти
                </Button>
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field
              data-invalid={fieldState.invalid}
              className="relative mb-0 text-muted-foreground"
            >
              <FieldGroup className="flex flex-row justify-between">
                <FieldLabel htmlFor={field.name}>Унікальний нікнейм</FieldLabel>
              </FieldGroup>
              <FieldGroup className="flex flex-row gap-3 items-center">
                <Input
                  {...field}
                ...
                />
                <Button
                  disabled={!fieldState.isDirty || savingField === "userName"}
                  onClick={saveField("userName")}
                  className="bg-card h-full border-border text-muted-foreground cursor-pointer hover:bg-transparent disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-xl font-medium text-sm transition-colors"
                  variant={"outline"}
                >
                  Зберегти
                </Button>
          ...
          render={({ field, fieldState }) => (
            ...
            >
              ...
              <FieldGroup className="relative flex flex-col gap-0 md:flex-row mb-2 items-center">
                ...
                />
                <div
                  className={`md:absolute ${emailVerified ? "text-green-500 bg-green-200" : "text-orange-500 bg-amber-200"}  text-xs font-medium rounded-lg border border-transparent w-full md:w-fit flex items-center gap-1 right-0 py-2 px-4 mr-px`}
                >
                  {emailVerified ? (
                    <>
                      <Check className="w-4" />
              ...
              <FieldGroup className="flex flex-row flex-wrap gap-3 md:gap-8">
                {!emailVerified && (
                  <Button
                    ...
                    className="bg-card h-full w-full md:max-w-fit border-border text-muted-foreground cursor-pointer hover:bg-transparent disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-xl font-medium text-sm transition-colors"
                    variant={"outline"}
                  >
                ...
                <Button
                  ...
                  className="bg-card h-full border-border w-full md:max-w-1/4 text-muted-foreground cursor-pointer hover:bg-transparent disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-xl font-medium text-sm transition-colors"
                  variant={"outline"}
                >
                  <Mail size={16} />
                  Змінити пошту
        ...
        {pendingEmail && (
          <div className="text-[13px] flex flex-col bg-orange-200/50 dark:bg-orange-700/20 text-muted-foreground mt-1.5 gap-2.5 px-4 py-3.25 rounded-lg border border-orange-100/50 dark:border-orange-900/30">
            <p>
              Очікує підтвердження зміни на{" "}
              <b className="text-primary">{pendingEmail}</b>. Перевірте поточну
              скриньку <b className="text-primary">({email})</b> і підтвердіть
              зміну, потім підтвердіть лист у новій скриньці.
            </p>
            <Button
              disabled={
                changingEmail ||
              ...
              onClick={handleChangeEmail}
              className="cursor-pointer w-fit text-[13px] text-primary flex py-1.75 px-3.5 rounded-lg items-center gap-2.25 bg-orange-100 hover:bg-orange-300 dark:bg-orange-900/20 border border-primary dark:hover:bg-orange-950 dark:border-orange-900/30"
            >
              <Send size={16} />
              Надіслати ще раз
            </Button>
      ...
      <div className="space-y-4 border-b border-secondary pt-6 pb-8">
        ...
        {!hasPassword && (
          <div className="p-4 bg-secondary rounded-xl text-sm text-muted-foreground">
            Ви авторизовані через сторонній сервіс. Для вашого акаунту пароль не
            використовується.
          </div>
        )}
        ...
        <Button
          onClick={hasPassword === true ? handleChangePassword : handleCreatePassword}
          disabled={hasPassword === true ? savingField === "password" : savingField ==="createPassword" }
      ...
      <div className="border-b cursor-not-allowed border-secondary pt-6 pb-8">
        ...
        <div className="space-y-4 opacity-50">
          <Field orientation="horizontal" className="w-full">
            <FieldContent>
              <FieldLabel
                className="font-normal text-muted-foreground text-sm pr-4"
      ...
      <div className="mt-12">
        ...
        <Button
          ...
          className="px-5 py-2.5 rounded-xl font-medium text-sm border border-red-200 text-red-600 h-full bg-transparent hover:bg-red-50 hover:border-red-300 cursor-pointer transition-all"
        >
          Видалити акаунт
        </Button>
